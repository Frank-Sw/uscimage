...
$endif$
$if(author)$
\author$if(short_author)$[
$for(short_author)$$short_author$$sep$ \and $endfor$
]$endif${$for(author)$$author$$sep$ \and $endfor$}
$endif$
$if(institute)$
\institute$if(short_institute)$[$short_institute$]$endif${$for(institute)$$institute$$sep$ \and $endfor$}
$endif$
\date{$date$}
$if(titlegraphic)$
